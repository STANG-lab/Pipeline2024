...
class UtteranceFeaturizer(Featurizer):
        ...
        for aggregate in all_aggregate_files:
            ...
            for n, l in df.groupby(["uid", "sentence_id"]):
                ...
                for i in range(0, len(sent_parse["words"])):
                    ...
                    token_id += 1
                sentence_id += 1
                sent_df = pd.concat([sent_df] + element_dfs, ignore_index=True)
            sent_df.to_csv(self.sent_feats / aggregate.name)
    ...
    # In[4]:
    def get_agg_sent_feats(self):
        # get all the files in the transcripts folder (the folder should just include transcripts)
        all_aggregate_files = self.sent_feats.glob("*")
        dependancy_tags = []
        dialog_tags = []
        semantic_tags = []
        ...
        for aggregate in all_aggregate_files:
            ...
            report_dfs.append(pd.DataFrame([entry]))
            # report_df = pd.concat([report_df, ], ignore_index=True)
        utt_feats_df = pd.concat(report_dfs, ignore_index=True)
        utt_feats_df.to_csv(self.feat_dir / "5_utterance_features.csv")
