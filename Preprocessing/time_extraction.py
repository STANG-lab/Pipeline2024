...
#         -v: provides container access to the specified volume ac_pipe that is visibl ithin the container as /ac_pipe
...
def get_time_features(audio_path, text_path, outdirs):
    path_to_audio, path_to_trans, pre_output_path, output_path, mega_path, log_path, features_path = set_paths(audio_path, text_path, outdirs)
    post_lab_path = output_path
    ...
    def is_unknown_speaker(speaker):
        ...
        valid_speakers = {"interviewer", "subject", "interviewer+subject",  "subject+interviewer", "other", "SPEAKER_00", "SPEAKER_01"}
        if speaker in valid_speakers:
            return False
        else:
            return True

    def label_lab_file(list_lab, list_mega, offset, file):
        def run_SAD():
            # make lists of files for processing
            list_of_audio = glob.glob(path_to_audio + '/*.wav')
            list_of_trans = glob.glob(path_to_trans + '/*.txt')

            print(len(list_of_audio), " audio files.")
            print(len(list_of_trans), " transcript files.")

            args = ["ldc-bpcsad", "--output-dir", pre_output_path, "--nonspeech", "0.250", "--speech", "0.250"]
                ...
                if lab[2] == 'speech':
                    ...
                    # EXCEEDS or PRECEDES AND EXCEDES megasegment
                        ...
                        # Overlaps with subsequent megasegment(s)
                            ...
                            new_list_lab.append([lab[0], list_mega[mega_iter][1] - offset, lab[2], 'speech', list_mega[mega_iter][2]])
                            for k in range(1, overlap):
                                new_list_lab.append([list_mega[mega_iter+k][0] - offset, list_mega[mega_iter+k][1] - offset, lab[2], 'speech', list_mega[mega_iter+k][2]])
                            new_list_lab.append([list_mega[mega_iter+overlap][0] - offset, lab[1], lab[2], 'speech', list_mega[mega_iter+overlap][2]])
    ...
    # Read each SAD output lab file and generate labeled lab files

    # In[27]:


    # list_of_lab_files = glob.glob(pre_output_path  '/*.lab')
    list_of_lab_files = pre_output_path.glob("*.lab")
    list_of_trans_files = path_to_trans.glob("*.txt")
    ...
    for lab_path in list_of_lab_files:
        ...
        trans_path = path_to_trans + '/' + lab_name + '.txt'
        ...
        if trans_path in list_of_trans_files:
            #FOR DEBUGGING ONLY, remove later TODO
            '''isHere = False
            if trans_path != '/Volumes/Alex_R_Music_ssD/Research/ac_pipe/data/remora_test/transcripts_by_task/remora_10308_T1_10308T151_v2023_SOC_Hinting.txt':
                continue
            else: isHere=True'''
    ...
    def log_problems():
        # This function is defined within the above because its variables are defined
    ...
    supp_features = [string for string in all_features if string not in primary_features]
    ...
    list_of_lab_files = post_lab_path.glob("*.lab")
    ...
    for file in list_of_lab_files:
        filename = file.split('/')[-1].split('.lab')[0]
        ...
        result = []
        for type in ['speech', 'pause', 'latency', 'NA_SKIP', 'NA_SPEAKER']:
            for speaker, group in labs_df[labs_df['type'] == type].groupby('speaker'):
    ...
    all_features_name = features_path / 'timing_features_all.tsv'
    primary_features_name = features_path / 'timing_features_primary.tsv'
    supp_features_name = features_path / 'timing_features_supp.tsv'
