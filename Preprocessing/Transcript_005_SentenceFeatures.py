...
# In[3]:
def get_sent_feats(outdirs):
    get_sent_csvs(outdirs)
    get_agg_sent_feats(outdirs)

def set_paths(outdirs):
    return outdirs["Features/word_aggregates"], outdirs["Features/sentence_features"], outdirs["Features"]
...
# In[4]:
def get_agg_sent_feats(outdirs):
    word_agg, sent_feat_dir, feat_dir = set_paths(outdirs)
    # get all the files in the transcripts folder (the folder should just include transcripts)
    all_aggregate_files = sent_feat_dir.glob("*")
    ...
    report_df.to_csv(feat_dir / "5_utterance_features.csv")
