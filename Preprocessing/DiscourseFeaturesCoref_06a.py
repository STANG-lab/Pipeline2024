...
# drive_discourselevel_coref_out_path = (
#     root_data + dicourse_feature_location + "6a_coreferences//"
# )
#
...
#
def clean_content(turn_content):
    ...
    def make_coref_csvs(self):
        ...
        for aggregate in self.word_agg.glob("*"):
            ...
            for index, row in df.iterrows():
                # This loop aggregates adjacent utterances by the same speaker
                if index == 0:
            ...
            for index, row in token_index_coref.iterrows():
                if index + 1 < len(token_index_coref):
                    if token_index_coref.at[index + 1, "token_coref"] == ":":
                        ...
                        coref_df = pd.concat(
                            [coref_df, pd.DataFrame([entry])], ignore_index=True
                        )
                else:
                    entry = {}
                    entry["speaker"] = current_speaker
            ...
            coref_df.to_csv(self.coref_dir / aggregate.name)

    def get_coref_features(self):
        # all_aggregate_files = os.listdir(drive_discourselevel_coref_out_path)
        all_aggregate_files = list(self.coref_dir.glob("*"))
            ...
            for each_cluster in clusters:
                ...
                for index, row in df.iterrows():
                    if type(row["clusters"]) != float:
                        cluster_list = [itm for itm in row["clusters"].split(",") if itm == each_cluster]
                ...
                filtered_cluster_df = cluster_df.iloc[1:, 0:].loc[
                    cluster_df["speaker"] == "Participant"
                ]
                if len(filtered_cluster_df) > 0:
                    ...
                    # print(cluster_df)
                    size_list.append(
                        len(cluster_df.loc[cluster_df["speaker"] == "Participant"])
                    )
                    for index, row in cluster_df.loc[
                        cluster_df["speaker"] == "Participant"
                    ].iterrows():
                    ...
                    for index, row in cluster_df.iterrows():
                        if index == 0:
                            ...
                            distance_list.append(
                                row["address"][0] - cluster_df.at[index - 1, "address"][1]
                            )
        ...
        report_df.to_csv(self.feature_folder / "6a_coref_features.csv")
