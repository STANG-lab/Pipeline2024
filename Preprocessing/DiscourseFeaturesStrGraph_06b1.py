...
class StructFeaturizer(Featurizer):
    def __init__(self, outdirs):
        ...
        # self.str_graph_dir = self.outdirs["Features/6_discourse_features/6b_struct_graphs/"]
        # self.word_aggs = self.outdirs['Features/word_aggregates']

        self.get_struc_graph_features()


    ### graph feature function ###
    def graph_features(self, aggregate, G_seq):
        entry = {}
        entry["uid"] = aggregate.name.split(".")[0]
    ...
    def get_struc_graph_features(self):
        ...
        for aggregate in self.word_aggs.glob("*"):
            ...
            # with open(
            #     self.str_graph_folder / aggregate.name, "w"
            #     drive_discourselevel_graph_out_path + aggregate.split(".")[0] + ".txt", "w"
            # ) as node_list:
            with open(self.str_graph_dir / aggregate.name, "w") as node_list:
            ...
            entry = self.graph_features(aggregate, G_seq)
            ...
            if len(node_sequence) > 40:
                ...
                while m < (len(node_sequence) - 30):
                    ...
                    dynamic_entry.append(self.graph_features(aggregate, G_seq30))
        ...
        df_strgfeatures.to_csv(
            self.feat_dir / "6b_sequential_graph_features.csv"
        )
