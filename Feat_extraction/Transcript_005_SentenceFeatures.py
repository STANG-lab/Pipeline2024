...
connective_raw_location = "Remora-2023//Batch-1//6_discourse_level_features//6d_connectives//raw_samples//"
...
for aggregate in all_aggregate_files:
    if aggregate.split('_')[5] in ['SOC', 'PIC', 'JOU', 'BTW']:
        ...
        sent_df = pd.DataFrame(columns=["uid","sentence_id" ,"token_id","content",])
        sentence_id = 0
        token_id = 0
        ...
        for n,l in df.groupby(["uid", "sentence_id"]):
            ...
            sent = l["content"].str.cat(sep =" ") # pasting together the original sentence
            sent = sent.replace('  ', ' ').replace(' .', '.').replace(' !', '!').replace(' ,', ',').replace(' ?', '?').replace(" ' " , "'").replace(" '", "'").replace("#", "").replace('  ', ' ')
            #print(sent)
        ...
        each_turn = []
        print(aggregate)
        with open(drive_wordlevel_out_path_connective+aggregate.split('.')[0]+'.txt', 'w') as sample_text:
            for n,l in sent_df.groupby(["sentence_id", "speaker"]):         
                for index, row in l.iterrows():
                    if l['speaker'].unique()[0] == current_speaker:
                        each_turn.append(row['content'])
                    else:
                        sample_text.write(' '.join(each_turn).replace('  ', ' ').replace(' .', '.').replace(' !', '!').replace(' ,', ',').replace(' ?', '?').replace(' - ', '-').replace(" ' " , "'").replace(" '", "'").replace("#", "").replace("  ", " "))
                        sample_text.write('\n')
...
for aggregate in all_aggregate_files:
    ...
    for index, row in df.iterrows():
        ...
        if row['dualog_tag'] not in dialog_tags:
            dialog_tags.append(row['dualog_tag'].replace(' ','').replace('/','').replace('(','').replace(')','').replace('-','').replace(',','').lower())
...
for aggregate in all_aggregate_files:
    ...
    for tag in dialog_tags:
        dialog_dic[tag.replace(' ','').replace('/','').replace('(','').replace(')','').replace('-','').replace(',','').lower()] = 0
    ...
    for each_sentence in df['sentence_id'].unique():
        ...
        dialog_tag = sentence_df['dualog_tag'].unique()[0].replace(' ','').replace('/','').replace('(','').replace(')','').replace('-','').replace(',','').lower()
    ...
    try:
        dependancy_dic_w = {key: (value / u_n_words)*100 for key, value in dependancy_dic.items()}
    except:
        dependancy_dic_w = {}
    try:
        semantic_dic_u = {key: value / u_n_utterance for key, value in semantic_dic.items()}
    except:
        semantic_dic_u = {}
    ...
    try:
        entry['u_n_statement'] = dialog_dic['statementnonopinion'] + dialog_dic['statementopinion']
    except:
        pass
    try:
        entry['u_s_statement'] = dialog_dic_s['statementnonopinion'] + dialog_dic_s['statementopinion']
    except:
        pass
...
for i in range(0, len(sent_parse['words'])):
    ...
    for j in range (0, len(sent_srl['verbs'])):
        srl_tag.append(sent_srl['verbs'][j]['tags'][i])
    print(sent_parse['words'][i],sent_parse['pos'][i], sent_parse['predicted_dependencies'][i], sent_srl['words'][i], srl_tag)
