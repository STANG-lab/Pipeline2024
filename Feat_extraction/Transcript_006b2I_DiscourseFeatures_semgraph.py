...
def srl_coref_embed(text):
  ...
  for sentence in somajo_sentences:
      ...
      instance = pd.DataFrame({'sentence_id': [n_sent], 'content': [content_sent.strip()]})
      sentence_df = pd.concat([sentence_df, instance], ignore_index=True)
      n_sent += 1
  for index, row in sentence_df.iterrows():
    sentence_df.at[index, 'dialogue_tag'] = speechact_model.predict_tag(row['content'])
  ###SRL and Coref
  token_index_srl = pd.DataFrame(columns=['dialogue_tag','token_index','token_index_utterance','utterance_index', 'token_srl','all_roles'])
  n1 = 0
  document = []
  n3 = 0
  all_srl = []
  for index, row in sentence_df.iterrows():
    ...
    token_index_srl_utterance = pd.DataFrame(columns=['token_index','token_index_utterance','utterance_index', 'token_srl','all_roles'])
    for each_token in srl_pred['words']:
      ...
      n2 = n2 +1
      n1 = n1 +1
    token_index_srl = pd.concat([token_index_srl,token_index_srl_utterance], ignore_index=True)
    n3 = n3 +1
  ...
  def bert_text_preparation(text, bert_tokenizer):
    ...
    indexed_tokens = bert_tokenizer.convert_tokens_to_ids(tokenized_text) # find token IDs
    segments_ids = [1]*len(indexed_tokens) # for formating and vectors matrix calculations
  ...
  for each_batch in sentence_df['bert_batch'].unique():
      token_index_bert_batch = pd.DataFrame(columns=['token_index', 'token', 'bert_embedding'])
      n= 0
      document = []
      for index, row in sentence_df.loc[sentence_df['bert_batch'] == each_batch].iterrows():
      ...
      for i in range(0, len(list_token_embeddings)):
          token_index_bert_batch.at[i, 'bert_embedding'] = list(list_token_embeddings[i].numpy())
      token_index_bert = pd.concat([token_index_bert, token_index_bert_batch], ignore_index=True)
  ...
  for index, row in coref_srl.iterrows():
    ...
    for index2, row2 in token_index_bert.iterrows():
      if row2['token_start'] >= row['token_start'] and row2['token_end'] <= row['token_end']:
  ...
  return sentence_df, coref_srl.drop(['token_start', 'token_end'], axis=1), sequence_alignment
        ...
        for index, row in tokens.iterrows():
            if index+1 < len(tokens):
                if tokens.at[index+1, 'token_coref'] == ':':
                    ...
                    final_df = pd.concat([final_df, pd.DataFrame([entry])], ignore_index=True)
            else:
                entry = {}
                entry['speaker'] = current_speaker
                entry['token_index'] = row['token_index']
                ...
                entry['bert_token'] = row['bert_token']
                final_df = pd.concat([final_df, pd.DataFrame([entry])], ignore_index=True)
        final_df.to_excel(dicourse_feature_location_semgraph+aggregate.split('.')[0]+".xlsx")
