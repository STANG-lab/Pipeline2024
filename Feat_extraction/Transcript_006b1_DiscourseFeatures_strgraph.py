...
drive_discourselevel_graph_out_path = root_data+dicourse_feature_location+"6b_graphs//6b1_strgraphs//"
...
def graph_features(aggregate, G_seq):
    ...
    try:
        entry['g_sq_degree'] = sum([d for (n,d) in nx.degree(G_seq)]) / float(G_seq.number_of_nodes())
    except:
        entry['g_sq_degree'] = 0
    try:
    ...
    for node1 in nx.nodes(G_seq):
        ...
        for node2 in nx.neighbors(G_seq, node1):
            #print('\t', node2)
            parallel_edge = parallel_edge + (G_seq.number_of_edges(u=node1, v=node2) - 1)
    ...
    for seedidx in range(0,200):
        ...
        G_rand = nx.gnm_random_graph(entry['g_sq_nodes'], entry['g_sq_edges'], seed=seed, directed=True)
        try:
    ...
    try:
        ...
        pass
    try:
        entry['g_sq_zdiameter'] = (entry['g_sq_diameter']-mean(lscc_rand))/stdev(lscc_rand)
    except:
        pass
    try:
...
for aggregate in all_aggregate_files:
    if aggregate.split('_')[5] in [ 'JOU', 'SOC', 'PIC']:
        ...
        df = df.loc[df['speaker'] == 'Subject'].copy().reset_index()[['sentence_id', 'token_id', 'content']]
        node_sequence = []
        ...
        with open(drive_discourselevel_graph_out_path+aggregate.split('.')[0]+'.txt', 'w') as node_list:
        ...
        df_strgfeatures = pd.concat([df_strgfeatures, pd.DataFrame([entry])], ignore_index=True)

df_strgfeatures.to_csv(root_data+'Remora-2023//Batch-1//features//6b1_sequential_graph_features.csv')
