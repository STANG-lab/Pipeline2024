...
vad_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//sentiment_dictionaries//VAD dictionary.csv").drop('Unnamed: 0', axis=1)
...
with open("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//sentiment_dictionaries//NRC-Emotion-Lexicon-Wordlevel-v0.92.txt", 'r') as sentiment_doc:
...
aoa_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//1_AoA_Prevalence.csv")
aoa_dic = {}
...
semd_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//2_SemD.csv")
semd_df['item'] = semd_df['item'].astype(str)
semd_dic = {}
for index, row in semd_df.iterrows():
    for feature in ['mean_cos','SemD']: #,'BNC_wordcount','BNC_contexts','BNC_freq','lg_BNC_freq'
        semd_dic[(row['item'].lower(), feature)] = row[feature]

calg_conc_dic = {}

calg_conc = pd.read_excel("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//3_CalgaryConcrete.xlsx")
...
rate_conc = pd.read_excel("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//4_ConcRate.xlsx")
...
prevalence_dic = {}
prevalence_df = pd.read_excel("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//5_prevalence.xlsx")
...
iconicity_dic = {}
iconicity_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//6_iconicity.csv")
...
sensorimotor = ['Auditory.mean', 'Gustatory.mean', 'Haptic.mean', 'Interoceptive.mean', 'Olfactory.mean', 'Visual.mean', 'Foot_leg.mean', 'Hand_arm.mean', 'Head.mean', 'Mouth.mean', 'Torso.mean']
sensorimotor_dic = {}
sensorimotor_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//7_sensorimotor.csv")
...
subtlex_dic = {}
subtlex_df = pd.read_excel("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//8_subtlex.xlsx")
...
taboo_dic = {}
taboo_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//9_taboo.csv")
...
glasgow_dic = {}
glasgow_df = pd.read_csv("C://Users//ANikzad//Desktop//Local_Pipeline//Transcript_Pipeline//lexical_dictionaries//10_glasgow.csv")
...
for aggregate in all_aggregate_files[0:1]:  # eliminate slice for debug
    if aggregate.split('_')[5] in ['SOC', 'PIC', 'JOU', 'BTW', 'FLU']:
        ...
        df2 = pd.DataFrame(columns= ['uid', 'speaker','sentence_id', 'token_id', 'content', 'wordfeature_id'])
        ...
        for index, row in df.iterrows():
            ...
            if len(token.split(' ')) == 1:
                df2 = pd.concat([df2, pd.DataFrame( [{ 'uid':row['uid'], 
                                                    'speaker':row['speaker'], 
                                                    'sentence_id':row['sentence_id'], 
                                                    'token_id': row['token_id'],
                                                    'wordfeature_id': new_index,
                                                    'content':row['content']}])], ignore_index=True)
                new_index += 1
                ...
                for each_token in token_split:
                    df2 = pd.concat([df2, pd.DataFrame( [{ 'uid':row['uid'], 
                                                    ...
                                                    'token_id': row['token_id'], 
                                                    'wordfeature_id': new_index,
                                                    'content':each_token}])], ignore_index=True)
                    new_index +=1
        ...
        # tokenized list
        df2["nltk.tokenized"] = df2.apply(lambda row: nltk.word_tokenize(str(row["word_lower"]).lower()) , axis = 1)
        df2["nltk.n_token"] = df2.apply(lambda row: len(row["nltk.tokenized"]) , axis = 1)

        # count stopwords per row:
        df2["nltk.n_stopword"] = df2.apply(lambda row: len(list(filter(lambda x: x in swords, row["nltk.tokenized"]))), axis = 1 )


        # df_no_nsv["nltk.n_stopword"] = df_no_nsv.apply(lambda row: row["word_lower"] in swords, axis = 1 )
        df2["is_stopword"] = df2.apply(lambda row: 1 if row["nltk.n_stopword"] > 0 else 0, axis=1)  


        # Create a Tokenizer with the default settings for English
        tokenizer = nlp.tokenizer
        # create tokenization array
        df2["sp.tokenized"] = df2.apply(lambda row: list(tokenizer(row["word_lower"])) if not isNaN(row["word_lower"]) else [] , axis = 1)
        ...
        pos_df = pd.DataFrame(columns=["wordfeature_id", "sp.token_text", "sp.lemma", "sp.pos", "sp.tag", "sp.shape", "sp.is_alpha"])

        for n,l in df2.groupby(["uid", "sentence_id"]):
            # print(l) # grouped df of this sentence
            sent = l["word_lower"].str.cat(sep =" ") # pasting together the original sentence
            # print("SENTENCE: " + sent)
            # do the POS tagging
            doc = nlp(sent) 
        ...
        df2 = df2.merge(pos_df, how = "left", left_on="wordfeature_id", right_on='wordfeature_id')
        ...
        for index, row in df2.iterrows():
            ...
            for sm in sensorimotor:
                df2.at[index, f"sm_{sm.lower().split('.')[0]}"] = mean(sensorimotor_dic[sm])
...
for aggregate in all_aggregate_files:
    ...
    l_me_sensory = mean([l_me_auditory, l_me_gustatory, l_me_haptic, l_me_interoception, l_me_olfaction, l_me_visual])
    ...
    l_n_dict = pos_counter.update({key.lower() : abs_conc_counter[key] for key in abs_conc_counter.keys()})
    ...
    each_report = pd.DataFrame([{'filename':filename, 'l_n_words': l_n_words, 'l_n_stopwords': l_n_stopwords, 'l_w_stopwords': l_w_stopwords,
        'l_n_syllable': l_n_syllable, 'l_w_syllable': l_w_syllable, 'l_mx_syllable': l_mx_syllable, 'l_md_aoa': l_md_aoa, 'l_mx_aoa': l_mx_aoa, 'l_md_semd': l_md_semd,
        'l_md_concrete': l_md_concrete, 'l_md_prevalence': l_md_prevalence, 'l_me_auditory': l_me_auditory, 'l_me_gustatory': l_me_gustatory, 'l_me_haptic': l_me_haptic,
        'l_me_interoception': l_me_interoception, 'l_me_olfaction': l_me_olfaction, 'l_me_visual': l_me_visual, 'l_me_footleg': l_me_footleg, 'l_me_handarm': l_me_handarm,
        'l_me_head': l_me_head, 'l_me_mouth': l_me_mouth, 'l_me_torso': l_me_torso, 'l_me_sensory': l_me_sensory, 'l_me_motor': l_me_motor, 'l_md_wordfreq': l_md_wordfreq,
        'l_md_contextdiversity': l_md_contextdiversity, 'l_w_anger': l_w_anger, 'l_w_anticipation': l_w_anticipation, 'l_w_disgust': l_w_disgust, 'l_w_fear': l_w_fear,
        'l_w_joy': l_w_joy, 'l_w_negative': l_w_negative, 'l_w_positive': l_w_positive, 'l_w_sadness': l_w_sadness, 'l_w_surprise': l_w_surprise, 'l_w_trust': l_w_trust,
        'l_me_valence': l_me_valence, 'l_me_arousal': l_me_arousal, 'l_me_dominance': l_me_dominance, 'l_me_imagibility': l_me_imagibility, 'l_me_gender': l_me_gender,
        'l_me_size': l_me_size, 'l_me_taboo': l_me_taboo, 'l_n_concrete': l_n_concrete, 'l_n_abstract': l_n_abstract, 'l_n_INTJ': l_n_INTJ, 'l_n_PROPN': l_n_PROPN,
        'l_n_NUM': l_n_NUM, 'l_n_VERB': l_n_VERB, 'l_n_NOUN': l_n_NOUN, 'l_n_ADV': l_n_ADV, 'l_n_ADJ': l_n_ADJ, 'l_n_ADP': l_n_ADP, 'l_n_PRON': l_n_PRON, 'l_n_AUX': l_n_AUX,
        'l_n_DET': l_n_DET, 'l_n_SCONJ': l_n_SCONJ, 'l_n_PART': l_n_PART, 'l_n_CCONJ': l_n_CCONJ, 'l_n_X': l_n_X,'l_w_concrete': l_w_concrete, 'l_w_abstract': l_w_abstract,
        'l_w_INTJ': l_w_INTJ, 'l_w_PROPN': l_w_PROPN, 'l_w_NUM': l_w_NUM, 'l_w_VERB': l_w_VERB, 'l_w_NOUN': l_w_NOUN, 'l_w_ADV': l_w_ADV, 'l_w_ADJ': l_w_ADJ, 'l_w_ADP': l_w_ADP,
        'l_w_PRON': l_w_PRON, 'l_w_AUX': l_w_AUX, 'l_w_DET': l_w_DET, 'l_w_SCONJ': l_w_SCONJ, 'l_w_PART': l_w_PART, 'l_w_CCONJ': l_w_CCONJ, 'l_w_X': l_w_X}])
