...
### Specify Source Folder
    # --- Remora
dicourse_feature_location_semgraph = root_data+"Remora-2023//Batch-1//6_discourse_level_features//6b_graphs//6b2_semgraphs//raw_df//"

### Specify Destination folder
    # --- Remora
dicourse_feature_location_semgraphfeatures = root_data+"Remora-2023//Batch-1//6_discourse_level_features//6b_graphs//6b2_semgraphs//refined_df//"

dicourse_feature_location_peripheryedges = root_data+"Remora-2023//Batch-1//6_discourse_level_features//6b_graphs//6b2_semgraphs//periphery_edgelist//"

dicourse_feature_location_coreedges = root_data+"Remora-2023//Batch-1//6_discourse_level_features//6b_graphs//6b2_semgraphs//core_edgelist//"
...
arguments = ['ARG0','C-ARG0','R-ARG0','ARG1', 'R-ARG1','C-ARG1', 'ARG2','R-ARG2', 'C-ARG2','ARG3','ARG4','ARG5',]
predicates = ['V', ]
manner = [ 'ARGM-ADV','ARGM-MNR', 'C-ARGM-MNR', 'R-ARGM-MNR', 'ARGM-PRD','ARGM-EXT', 'ARGM-COM','ARGM-ADJ',]
setting = ['ARGM-TMP','ARGM-LOC','R-ARGM-LOC','R-ARGM-TMP','ARGM-DIR','ARGM-GOL',]
cause = ['ARGM-CAU','ARGM-PRP', 'ARGM-PNC',]
...
def graph_feature(aggregate, G, g_type):
    ...
    try:
        entry[f'g_{g_type}_degree'] = sum([d for (n,d) in nx.degree(G)]) / float(G.number_of_nodes())
    except:
    ...
    try:
        entry[f'g_{g_type}_aspl'] = nx.average_shortest_path_length(nx.to_undirected(G))
         ...
         pass
    try:
        entry[f'g_{g_type}_lscc'] = len(max(nx.strongly_connected_components(G), key=len))
    except:
        entry[f'g_{g_type}_lscc'] = 0
    try:
        entry[f'g_{g_type}_ncc'] = len(sorted(nx.connected_components(nx.Graph(G)), key=len, reverse=True))
    except:
        entry[f'g_{g_type}_ncc'] = 0
    try:
        entry[f'g_{g_type}_lcc'] = len(max(nx.connected_components(nx.Graph(G)), key=len))
    except:
    ...
    for seed in range(0,200):
        G_rand = nx.gnm_random_graph(entry[f'g_{g_type}_nodes'], entry[f'g_{g_type}_edges'], seed=seed,directed=True)
        try:
        ...
        try:
            ...
            pass
        try:
            lscc_rand.append(len(max(nx.strongly_connected_components(G_rand), key=len)))
        except:
            pass
        try:
            lcc_rand.append(len(max(nx.connected_components(nx.Graph(G_rand)), key=len)))
        except:
            pass
        try:
            ncc_rand.append(len(sorted(nx.connected_components(nx.Graph(G_rand)), key=len, reverse=True)))
        except:
            pass
    try:
        entry[f'g_{g_type}_zlscc'] = (entry[f'g_{g_type}_lscc']-mean(lscc_rand))/stdev(lscc_rand)
    except:
        pass
    try:
        entry[f'g_{g_type}_zncc'] = (entry[f'g_{g_type}_ncc']-mean(ncc_rand))/stdev(ncc_rand)
    except:
        pass
    #try:
    entry[f'g_{g_type}_zlcc'] = (entry[f'g_{g_type}_lcc']-mean(lcc_rand))/stdev(lcc_rand)
    #except:
     #   entry[f'g_{g_type}_zlcc'] = np.nan
    try:
        entry[f'g_{g_type}_zdiameter'] = (entry[f'g_{g_type}_diameter']-mean(diameter_rand))/stdev(diameter_rand)
    except:
        pass
    try:
        entry[f'g_{g_type}_zaspl'] = (entry[f'g_{g_type}_aspl']-mean(aspl_rand))/stdev(aspl_rand)
    except:
        pass
    try:
        entry[f'g_{g_type}_zcc'] = (entry[f'g_{g_type}_cc']-mean(cc_rand))/stdev(cc_rand)
    except:
        pass
    return entry
...
for aggregate in all_aggregate_files[0:2]:
    if aggregate.split('_')[5] in ['JOU', 'PIC']:
        ...
        df = df[['utterance_index','dialogue_tag','token_index', 'token_srl', 'all_roles', 'n_clusters', 'clusters']]
        tokens_dic = {}
        clusters = []
            ...
            for i in range(0, n_roles):
                ...
                for each_role in utterance_roles:
                    if each_role in all_roles:
                        ...
                        try:
                            role_tokens_content.append(clusters_dic[(role_tokens[0], role_tokens[-1])])
                        ...
                        entry[each_role] = extract_lemma(core_extract(' '.join(role_tokens_content).replace(" ' ", "'").replace(" '", "'").replace(" ,", ","))) 
                if len(entry) > 2:
                    role_df = pd.concat([role_df, pd.DataFrame([entry])], ignore_index=1)
    
        role_df.to_csv(dicourse_feature_location_semgraphfeatures+aggregate.split('.')[0]+'.csv')
        ...
        for index, row in role_df.iterrows():
            ...
            if row['V'] not in ["'", ''] and type(row['V']) == str:
                ...
                for each_argument in arguments:
                    try:
                        if type(row[each_argument]) == str and row[each_argument] != ' ':
                            for element in row[each_argument].split(' '):
                                G_core.add_edge(row["V"], element, weight=1, category = each_argument, sentence= sentence_id)
                    except:
                        pass
                            ...
                            if type(row['ARG1']) == str:
                                for each_actor in row['ARG0'].split(' '):
                                    for each_patient in row['ARG1'].split(' '):
                                        G_core.add_edge(each_actor, each_patient, weight=1 , category = 'action', sentence= sentence_id)
                    ...
                    try:
                        if type(row[each_adjunct]) == str:
                            for element in row[each_adjunct].split(' '):
                                G_periphery.add_edge(element, row["V"], weight=1, category = each_adjunct,sentence= sentence_id)
                    except:
                        pass      
           
        nx.write_edgelist(G_core, dicourse_feature_location_coreedges+aggregate.split('.')[0]+".txt", data=['weight', 'category'])
        nx.write_edgelist(G_periphery, dicourse_feature_location_peripheryedges+aggregate.split('.')[0]+".txt", data=['weight', 'category'])
        ...
        if len(sentence_list) > 4:
            ...
            while m+2 < len(sentence_list):
                ...
                dynamic_entry_cr.append(graph_feature(aggregate, G_core_dynamic,'3cr1'))
                dynamic_entry_pr.append(graph_feature(aggregate, G_periphery_dynamic,'3pr1'))
...
nx.write_edgelist(G_core, "C://Users//ANikzad//Desktop//test.txt", data=['weight', 'category'])
...
try:
    entry['core_ad'] = sum([d for (n,d) in nx.degree(G_core)]) / float(G_core.number_of_nodes())
except:
    entry['core_ad'] = 0
...
try:
    ...
    entry['core_lscc'] = 0
try:
    entry['core_ncc'] = len(sorted(nx.connected_components(nx.Graph(G_core)), key=len, reverse=True))
except:
    entry['core_ncc'] = 0
...
try:
    entry['periphery_ad'] = sum([d for (n,d) in nx.degree(G_periphery)]) / float(G_periphery.number_of_nodes())
except:
    entry['periphery_ad'] = 0
entry['periphery_gd'] = nx.density(G_periphery)
try:
    entry['periphery_lscc'] = len(max(nx.strongly_connected_components(G_periphery), key=len))
except:
    entry['periphery_lscc'] = 0
try:
    entry['periphery_ncc'] = len(sorted(nx.connected_components(nx.Graph(G_periphery)), key=len, reverse=True))
except:
